...
public class DBCmd {
    ...
    public static ICCObject parser_command(IDB db, String fid, String aid, ICCObject row) throws Exception {
        ...
        while (m.find()) {
            ...
            if (!item.startsWith("rem")) {
                if (dao.has(item)) {
                    ...
                    DBCmdItem.process_item(db, sql, model, row, item);
                }
            }
        }
        m.appendTail(sql);
    ...
    public static ICCObject parser_command(IDB db, String cmd, ICCObject row) throws Exception {
        ICCObject model = new JOObject();
        model.set(CCConst.dp_params, new JOList());     
        System.out.println(cmd);
        Matcher m = p.matcher(cmd);
